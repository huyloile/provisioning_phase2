-module(test_133_mobi_direct_callback).
...
test_mobi_access() ->
    [{title, "MOBI Access Direct Callback no roaming"}] ++
        init("mobi",?IMSI_OK,"") ++
	[
	 "Connect to #133# - Mobicarte, direct callback whith destination number -> NO ROAMING: not allowed",
	 {ussd2,
	  [
	...
	[
	 "Connect to #133# - Mobicarte, direct callback whithout destination number -> NO ROAMING: not allowed",
	 {ussd2,
	  [
    ...
    [{title, "MOBI Access Direct Callback with roaming"}] ++
        %% SUCCESS
        init("mobi",?IMSI_OK,?NOCAMEL_VLR) ++
...
test_mobi_direct_callback() ->
    [{title, "Mobicarte Direct Callback"}] ++

	%% SUCCESS
	init("mobi",?IMSI_OK,?MING_VLR) ++
	test_direct_callback(with_dest,
			     "PRISM OK: succes",
			     "0012345678901",
			     "Vous allez bientot recevoir un appel et etre mis en relation avec votre correspondant au bout de quelques secondes.")++
	...
	%% ERROR2 : epuise
	init("mobi",?IMSI_EPUISE,?MING_VLR) ++
	test_direct_callback(with_dest,
			     "PRISM ERROR2: epuise",
			     "0012345678901",
	...
	%% ERROR4 : pb crédit
	init("mobi",?IMSI_CREDIT,?MING_VLR) ++
	test_direct_callback(with_dest,
	...
	%% ERROR7
	init("mobi",?IMSI_OK,?BAD_MING_VLR) ++
	test_direct_callback(without_dest,
...
test_direct_callback(with_dest, Title, Dest, Expect) ->
    [
     "Connect to #133# - Mobicarte, direct callback whith destination number -> "++Title,
     {ussd2,
      [
...
test_direct_callback(without_dest, Title, Dest, Expect) ->
    [
     "Connect to #133# - Mobicarte, direct callback whithout destination number -> "++Title,
     {ussd2,
      [
       {send, ?DIRECT_CALLBACK_CODE},
